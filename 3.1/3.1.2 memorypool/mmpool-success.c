...
struct mp_large_s {
	struct mp_large_s *next;	// 链表下一个节点，构成大内存块管理链表
	void *alloc;			// 实际大内存的malloc分配地址（核心）
};

struct mp_node_s {

	unsigned char *last;	// 节点内当前可用内存的起始地址（核心：分配时从这里开始）
	unsigned char *end; 	// 节点内内存的结束地址（核心：标记节点的内存边界）
	
	struct mp_node_s *next; 	// 链表下一个小内存节点，构成小内存节点链表
	size_t failed; 				// 该节点分配失败的次数（优化：用于切换current指针）
};

struct mp_pool_s {

	size_t max;					// 小/大内存的分配阈值（核心：小于等于为小内存，大于为大内存）

	struct mp_node_s *current;	// 当前可用的小内存节点（优化：避免从头遍历链表）
	struct mp_large_s *large;	// 大内存块管理链表的头指针

	struct mp_node_s head[0];	// 小内存节点链表的头节点（柔性数组，无占用内存，仅作链表头）
